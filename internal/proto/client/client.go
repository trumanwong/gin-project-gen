...
var CmdClient = &cobra.Command{
	...
	Long:  "Generate the proto client code. Example: gin-project-gen proto client helloworld.proto",
	Run:   run,
}
...
func init() {
	if protoPath = os.Getenv("GIN_PROTO_PATH"); protoPath == "" {
...
func run(_ *cobra.Command, args []string) {
	...
	if err = look("protoc-gen-go", "protoc-gen-go-grpc", "protoc-gen-go-gin", "protoc-gen-openapi"); err != nil {
		// update the gin plugins
		cmd := exec.Command("gin-project-gen", "upgrade")
...
func generate(proto string, args []string) error {
	...
	inputExt := []string{
		...
		"--go-gin-errors_out=paths=source_relative:.",

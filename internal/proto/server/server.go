...
var CmdServer = &cobra.Command{
	...
	Long:  "Generate the proto server implementations. Example: gin-project-gen proto server api/xxx.proto --target-dir=internal/services",
	Run:   run,
}
var targetDir string

func init() {
	CmdServer.Flags().StringVarP(&targetDir, "target-dir", "t", "internal/services", "generate target directory")
}

func run(_ *cobra.Command, args []string) {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "Please specify the proto file. Example: gin-project-gen proto server api/xxx.proto")
		return
	}
